...
export const PokemonDetailPage = ({ pokeId, pokeName, t, isDark, onBack, backLabel = 'Recherche' }) => {
  ...
  return (
    <div className={`min-h-screen ${isDark ? 'bg-[#1c1c1e]' : 'bg-white'}`}>
      ...
      <button
        onClick={onBack}
        className={`fixed z-20 left-4 w-10 h-10 rounded-full flex items-center justify-center backdrop-blur-sm ${isDark ? 'bg-white/15 text-white' : 'bg-white/80 text-gray-900'}`}
