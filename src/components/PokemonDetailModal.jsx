  ...
  return createPortal(
    ...
    >
      ...
      >
        ...
        <button
          onClick={handleClose}
          className={`absolute top-2 right-4 w-11 h-11 rounded-full flex items-center justify-center backdrop-blur-xl ${isDark ? '' : 'border border-white/20'} ${isDark ? '' : 'shadow-[0_4px_24px_rgba(0,0,0,0.12)]'} z-10 ${isDark ? 'bg-white/10 text-white' : 'bg-white/60 text-gray-900'}`}
        ...
        {!loading && !error && data && (
          ...
          >
            ...
            <div className="px-5 pt-4 pb-0">
              <p className={`text-sm font-mono font-semibold ${isDark ? 'text-gray-500' : 'text-gray-400'}`}>#{String(data.displayId ?? data.id).padStart(4, '0')}</p>
